...
export default {
  ...
  methods: {
    newMessage() {
      this.checkInput();
      if(this.error.length === 0)this.$emit('newMessage', this.email, this.message, this.name);
    },
    checkInput(){
      this.error = [];
      //email
      ...
      //message
      if(this.message.length < 3 || this.message.length > 1000)this.error.push('message');
      //name
...
<template>
  ...
  <div class="cont-message">
    ...
    <form @submit.prevent="newMessage">
      ...
      <p v-if="error.includes('message')" class="mt-1 text-danger">il messaggio deve contenere almeno 3 e massimo 1000 caratteri</p>
